...
1. **Scope:** `manifest.json` (Philosophy, Phases, and Personas).
...
- **Plan First:** Complex tasks require a plan in `dev-plans/` before execution.
...
- **Execution:** `/plan`, `/secure` (The Sentinel), `/build` (The Executor), `/vibe`, `/test`, `/audit`, `/update`, `/yap`.
